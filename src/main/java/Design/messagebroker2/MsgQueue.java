...
public class MsgQueue {
	...
	public void retryFailedMg() {
		synchronized (MUTEX) {
			for (Entry<Topic, List<MsgQueueClient>> entry : publishersMap.entrySet()) {
				RetryFailedMsgs retryFailedMsgs = new RetryFailedMsgs(entry.getKey(), msgCount);
				retryFailedMsgs.retryMsgs(entry.getValue());
			}
		}
	}
